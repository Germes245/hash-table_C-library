...
size_t str_hash_table_chain_has_couple_with_key(str_hash_table_chain *chain, char* key){ //если есть, то индекс структуры, иначе 0
    //printf("длина %d\n", chain->length);
    for(size_t i = 0; i < chain->length; i++){
        //printf("%d %s %s\n", i, chain->array[i].key,key);
...
void str_hash_table_chain_put(str_hash_table_chain *chain, char* key, char* value){
    if(chain->length){ // если в цепочке что-то есть
    ...
    else{ // если в цепочке ничего нет
